/**
 * Бэктест стратегии DonchianBreakout для фьючерсов MOEX.
 * Использует 1h свечи, расчёт PnL в рублях.
 */

import { donchianBreakoutStrategy } from '../market/donchianBreakoutStrategy.js';
...
function closeTrade(
  ...
  return { ...trade, exitPrice, exitTime, pnl, reason };
}
...
export async function runBacktest(
  ...
  for (const candle of candles) {
    ...
    const signalResult = donchianBreakoutStrategy.getSignal(ticker);
    ...
    if (!openTrade && (signalResult.signal === 'LONG' || signalResult.signal === 'SHORT')) {
      ...
      const confirmed = donchianBreakoutStrategy.confirmEntry(ticker, side);
      ...
      const ctx = donchianBreakoutStrategy.getContext(ticker);
      ...
      diagnostics.push({
        ...
        entryPrice,
        atr,
        channelHigh: ctx?.channelHigh ?? 0,
        channelLow: ctx?.channelLow ?? 0,
        stopDistance,
  ...
  if (!silent) {
    console.log('================ DONCHIAN BREAKOUT BACKTEST (MOEX) ================');
    ...
    const byMonth = new Map<
      string,
      { wins: number; losses: number; pnl: number; stopCount: number; winSum: number; lossSum: number }
    >();
    ...
    if (sortedMonths.length) {
      console.log('--- По месяцам (Winrate % | PnL ₽ | STOP | avg win/loss) ---');
      for (const [month, row] of sortedMonths) {
        ...
        const avgW = row.wins ? (row.winSum / row.wins).toFixed(2) : '-';
        const avgL = row.losses ? (row.lossSum / row.losses).toFixed(2) : '-';
        console.log(
          `  ${month}: ${wr}% (W:${row.wins} L:${row.losses}) | PnL: ${pnlStr} ₽ | STOP:${row.stopCount} | avg win: ${avgW} avg loss: ${avgL}`
        );
      }
    }
  }
