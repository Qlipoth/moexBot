/**
 * Бэктест стратегии RSI Momentum по всем тикерам используя кэшированные свечи (без API Tinkoff).
 * Требует предварительного запуска: pnpm run download
 *
 * Использование: pnpm run backtest:rsi [--json]
...
import {
  runBacktest,
  type BacktestResult,
} from '../src/backtest/rsiMomentumBacktest.js';
...
async function main(): Promise<void> {
  ...
  console.log(`\n📊 RSI Momentum бэктест (кэш) | Тикеры: ${tickers.join(', ')}\n`);
  ...
  if (outputJson) {
    const bestTicker = results.reduce(
      (best, r) => (!best || r.pnl > best.pnl ? r : best),
      results[0] as BacktestResult | undefined
    );
    const worstTicker = results.reduce(
      (worst, r) => (!worst || r.pnl < worst.pnl ? r : worst),
      results[0] as BacktestResult | undefined
    );
    ...
    const summary = {
      ...
      description:
        'Моментум стратегия на выходе RSI(14) из зон экстремума (35/65) с трендовым фильтром EMA(20)',
      keyParameters: {
        rsiPeriod: 14,
        rsiLongLevel: 35,
        rsiShortLevel: 65,
